...
export function registerMiniOverlayHandlers(_ctx: IpcHandlerContext): void {
  ...
  // Hiding the overlay is sufficient — macOS and Windows will automatically
  // activate the next app in the window order.
  //
  // NOTE: Previously this called app.hide() on macOS which hid ALL windows
  // (including the main window) and removed the app from dock + Cmd+Tab.
  // That was the root cause of the "app disappears from dock" bug.
  // --------------------------------------------------------------------------
